...
我们还修改了ATOC,使其适用于连续作用和异构试剂. 结果显示在表1和2的最后一行.ATOC在流量控制任务中效果很好,但在路由和wifi任务中表现不佳. 主要原因是ATOC根据智能体之间的距离进行分组.同时ATOC发送的消息更多.
